...
public class DemoUI extends UI {
	...
	protected void init(final VaadinRequest request) {
		...
		((GeneratedPropertyContainer) grid.getContainerDataSource())
			.addGeneratedProperty("_multiCaptionGen", new PropertyValueGenerator<DummyClass>() {
		...
		grid.getColumn("_multiCaptionGen")
			.setRenderer(new ComboBoxMultiselectRenderer<DummyClass>(DummyClass.class, getItemList(), "id", "name", 5,
					"input prompt2", "select all2", "clear2", "multi", new EditableRendererEnabled<DataSourceClass>() {
						...
						@Override
						public String apply(DummyClass t) {
		...
		grid.setColumns("calc", "km", "bar", "actions", "single", "today", "yes", "foo", "_multi", "singleCaptionGen",
						"_multiCaptionGen");
		grid.setColumnOrder("calc", "km", "bar", "single", "_multi", "singleCaptionGen", "_multiCaptionGen", "today",
							"yes", "foo", "actions");
	}
	...
	public BeanItemContainer<DataSourceClass> getDataSource() {
		final BeanItemContainer<DataSourceClass> container = new BeanItemContainer<DataSourceClass>(
				DataSourceClass.class);

		// Populate data
		for (int i = 0; i <= 30; ++i) {
			container.addBean(new DataSourceClass("0,1", "foo", i, i / 5.0d, new Date(), true,
					new DummyClass(1L, "one"), new HashSet<DummyClass>() {
						{
							add(new DummyClass(1L, "one"));
							add(new DummyClass(8L, "eight"));
						}
	...
	public class DataSourceClass {
		...
		public DataSourceClass(final String actions, final String foo, final Integer bar, final Double km,
				final Date today, final Boolean yes, final DummyClass single, final HashSet<DummyClass> multi,
				final DummyClass singleCaptionGen, final HashSet<DummyClass> multiCaptionGen) {
		...
		public void setSingleCaptionGen(DummyClass singleCaptionGen) {
		...
		public void setMultiCaptionGen(HashSet<DummyClass> multiCaptionGen) {
